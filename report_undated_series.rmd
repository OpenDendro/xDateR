...
Highest overall correlation: `r round(rBest, 2)` with dates `r firstBest` to `r lastBest`
...
```
library(dplR)
rwl    <- read.rwl("`r fname1`")
undated <- read.rwl("`r fname2`")
fo <- xdate.floater(rwl,
                    series      = undated[, "`r floaterObject$series.name`"],
                    series.name = "`r floaterObject$series.name`",
                    min.overlap = `r undatedParams$seg.length`,
                    prewhiten   = `r undatedParams$prewhiten`,
                    biweight    = `r undatedParams$biweight`,
                    method      = "`r undatedParams$method`",
                    make.plot   = TRUE,
                    verbose     = TRUE)
```
